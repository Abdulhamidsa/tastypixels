...
const useFetchData = () => {
  ...
  const fetchData = async (page) => {
    try {
      ...
      if (isAuthenticated) {
        const userResponse = await fetchWithTokenRefresh("https://cre8ify-backend-develop-production.up.railway.app/users/profile");
        if (!userResponse.ok) {
      ...
      if (isAuthenticated && userData) {
        const postsResponse = await fetchWithTokenRefresh(`https://cre8ify-backend-develop-production.up.railway.app/recipes/all-posts?page=${page}`);
        if (!postsResponse.ok) {
